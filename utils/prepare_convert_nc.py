...
with open(convert_file, 'w') as f:
    for i in range(n_line):
        ...
        if line.startswith("!$TODO Add here 7"):
            for j in range(nvar):
                f.write("call getvar_real_2d(ncid, '" + var_set[j] + "'," +
                        var_abbr_set[j] + "," + var_miss_set[j] + ")\n")
                f.write("call getvar_real_2d(ncid, '" + var_set[j] + "_ERR" +
                        "'," + var_err_set[j] + "," + var_miss_set[j] + ")\n")
            added = 7
            continue
        ...
        if line.startswith("!$TODO Add here 8"):
            ...
            for j in range(nvar):
                f.write("call getvar_int_2d(ncid, '" + var_set[j] + "QCR" +
                        "', " + var_qc_set[j] + ")\n")
            f.write("else\n")
            for j in range(nvar):
        ...
        if line.startswith("!$TODO Add here 9"):
            ...
            for j in range(nvar - 1):
                f.write("  " + var_abbr_set[j] + "(n,k) /= " +
                        var_miss_set[j] + " .and. " + var_qc_set[j] +
                        "(n,k) == 0 .and. &\n")
            f.write("  " + var_abbr_set[-1] + "(n,k) /= " + var_miss_set[-1] +
                    " .and. " + var_qc_set[-1] + "(n,k) == 0) then\n")
            for j in range(nvar):
                f.write(
                    ...
                    var_err_set[j] + "(n,k)/3*inflation_coefficient, oday, osec, qc, obs)\n")
                f.write(
