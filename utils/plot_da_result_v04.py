...
class DaResults(object):
    def __init__(self, config_nml):
        ...
        self.obs_pflotran_var_set  = self.configs["obspara_set_cfg"]["obs_pflotran_set"]
        ...
        if isinstance(self.obs_pflotran_var_set, str):
            self.obs_pflotran_var_set = [self.obs_pflotran_var_set]
        if isinstance(self.para_var_set, str):
            self.para_var_set = [self.para_var_set]
        self.pflotran_var_set = self.obs_pflotran_var_set + self.para_var_set
    ...
    def setup(self):
        ...
        obs_pflotran_var_set = self.obs_pflotran_var_set
        ...
        with open(self.reconditioned_file, "r") as f:
        ...
        with Dataset(dart_prior_all_ens_file, 'r') as root_prior:
            ...
            # Get the cell ids
            cell_ids = {"para": root_prior.variables['para_cell_ids'][:][0,0,:], 
                        "state": root_prior.variables['state_cell_ids'][:][0,0,:]}
        ...
        self.cell_ids, self.reconditioned_cellids = cell_ids, all_reconditioned_cellids
