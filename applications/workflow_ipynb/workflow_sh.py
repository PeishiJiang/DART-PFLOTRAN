...
app_dir = sys.argv[1]   # the application repository
assim_window = int(sys.argv[2])   # the assimilation window (second)
mda_iter = int(sys.argv[3])   # the iteration number
rescaled = bool(int(sys.argv[4]))
print(rescaled == True)
# sys.exit()
...
ncore_da = 20  # The number of MPI cores used by DART
...
app_parent_dir = os.path.join(dart_pf_dir, "applications")    # The template for application folder
temp_app_dir = os.path.join(app_parent_dir, "1dthermal")      # The template for application folder
...
para_prior_rescaled = rescaled  # The assumed distribution to be sampled
...
da_cfg["assim_window_seconds"] = assim_window  # assimilation time window/step  (second)
...
#  - the start and end time of the current assimilation time window (obtained based on the previous settings)
...
if mda_iter == 1:
    ...
    raise Exception("Unsupported number of iterations {}.".format(mda_iter))
