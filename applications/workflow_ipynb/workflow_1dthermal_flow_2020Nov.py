...
temp_app_dir = "/home/jian449/DART/manhattan/models/pflotran/applications/1dthermal_flow"          # The template for application folder
app_dir      = "/home/jian449/DART/manhattan/models/pflotran/applications/1dthermal_parastate_1month_sortobsin_3mda"          # The application folder name
...
files_cfg["material_id_file"] = None

# Observation file
files_cfg["obs_nc_original_file"] = os.path.join(pflotran_in_dir, "obs_pflotran_original.nc")
files_cfg["obs_nc_file"] = os.path.join(pflotran_in_dir, "obs_pflotran_clipped.nc")
...
files_cfg["pflotran_in_file"] = os.path.join(pflotran_in_dir, "pflotran.in")
files_cfg["pflotran_out_prefix"] = "pflotran"
files_cfg["pflotran_out_file"] = os.path.join(pflotran_out_dir, "pflotranR[ENS].h5")
files_cfg["pflotran_restart_file"] = os.path.join(pflotran_out_dir, "pflotranR[ENS]-restart.chk")
...
files_cfg["use_obs_tecfile_for_prior"] = False
...
if not para_homogeneous:
    ...
    para_material_id_set = [None]
    para_hdf_dataset_name_set = para_set
    para_isotropic_set = [False]
    para_anisotropic_ratio_set = [1]
...
para_sample_method_set = ["normal"]  # the parameter sampling method, including: SGS, US, SIS, rescale, normal, lognormal, truncated_normal, uniform
    
# The statistics of the parameters
# The index order follows para_set
para_min_set  = [-5.]  # The minimum values (-99999 means no lower bound limit)
para_max_set  = [5.]  # The maximum values (99999 means no upper bound limit)
para_mean_set = [0.]  # The mean values
para_std_set  = [.5]  # The standard deviation values

# Others
para_take_log_set = [False] # Indicate whether the parameters should be estimated based on its original value or log value
...
#     time_cfg["last_obs_time_seconds"] = time_cfg["first_obs_time_seconds"] + 5*da_cfg["assim_window_seconds"]
...
da_cfg["obs_error"] = [0.05]       # the observation error (with the order corresponding to the obs_set)
...
notebook_file = 'workflow_1dthermal.ipynb'
script_file   = 'workflow_1dthermal_2020Nov'
...
get_ipython().system(' jupyter nbconvert --to script $notebook_file --output $script_file')
