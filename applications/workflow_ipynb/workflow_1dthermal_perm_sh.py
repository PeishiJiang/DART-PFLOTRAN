...
app_dir = sys.argv[1]
mda_iter = int(sys.argv[2])
sort_obs_inc = bool(int(sys.argv[3]))
...
ncore_da = 50  # The number of MPI cores used by DART
ncore_pf = 50  # The number of MPI cores used by PFLOTRAN
ngroup_pf= 50  # The number of group used by stochastic running in PFLOTRAN
...
app_parent_dir = os.path.join(dart_pf_dir, "applications")
temp_app_dir = os.path.join(app_parent_dir, "1dthermal_perm")  # The template for application folder
app_dir      = os.path.join(app_parent_dir, app_dir)   # The application folder name
...
para_min_set  = [-13]  # The minimum values (-99999 means no lower bound limit)
para_max_set  = [-9]  # The maximum values (99999 means no upper bound limit)
para_mean_set = [-11]  # The mean values
para_std_set  = [1]  # The standard deviation values
para_dist_set = ["lognormal"]  # The assumed distribution to be sampled
para_prior_rescaled = False  # Whether the prior is generated by rescaling the posterior
para_take_log = True # Indicate whether the parameters should be estimated based on its original value or log value
use_default_para_initial = True # Indicate whether using the default parameter ensemble as initial

para_resampled_set = ['PERMEABILITY']   # The parameters to be resampled at each time step
...
configs["obspara_set_cfg"] = {"obs_set": obs_set, "para_set": para_set,
                              "para_take_log": para_take_log, "use_default_para_initial":use_default_para_initial,
...
# If the assimilation window is fixed, then use assimilation window size through assim_window_size
# otherwise, use assimilation window list
# Assimilation window
da_cfg["assim_window_fixed"]   = False # whether the assimilation window is fixed (day)
...
da_cfg["assim_window_list"] = [2., (200*300+10)/86400.] # day
...
obs_start   = datetime(2017,3,30,0,0,0)
...
assim_tools_nml = {"filter_kind":2, "sort_obs_inc": sort_obs_inc}
