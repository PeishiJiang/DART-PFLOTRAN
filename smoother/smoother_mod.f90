! This smoother is revised from DART filter_mod.f90.
!
! $Id: smoother_mod.f90 13137 2020-03-08 peishi.jiang@pnnl.gov $
...
use utilities_mod,         only : register_module,  error_handler, E_ERR, E_MSG, E_DBG,       &
                                  ...
                                  set_multiple_filename_lists, find_textfile_dims, set_output

use assim_model_mod,       only : static_init_assim_model, get_model_size,                    &
                                  end_assim_model,  pert_model_copies, get_closest_state_time_to

! use assim_tools_mod,       only : filter_assim, set_assim_tools_trace, test_state_copies
use assim_smoother_tools_mod,       only : smoother_assim, set_assim_tools_trace, test_state_copies
...
use ensemble_manager_mod,  only : init_ensemble_manager, end_ensemble_manager,                &
                                  ...
                                  map_pe_to_task, map_task_to_pe, prepare_to_update_copies,  &
...
!           filter_main
public :: smoother_sync_keys_time, &
          smoother_set_initial_time, &
          smoother_main
...
character(len=128), parameter :: revdate  = "$Date: 2019-04-30 08:48:00 -0700 (Tue, 30 Apr 2019) $"
...
call smoother_initialize_modules_used() ! static_init_model called in here
...
if(ens_size < 2) then
   ...
   call error_handler(E_ERR,'smoother_main', msgstring, source, revision, revdate)
endif

! informational message to log
write(msgstring, '(A,I5)') 'running with an ensemble size of ', ens_size
call error_handler(E_MSG,'smoother_main:', msgstring, source, revision, revdate)
...
if (has_cycling .and. write_all_stages_at_end) then
   call error_handler(E_ERR,'smoother:', &
         'advancing the model inside smoother and writing all state data at end not supported', &
          source, revision, revdate, text2='delaying write until end only supported when advancing model outside smoother', &
...
call smoother_setup_obs_sequence(seq, in_obs_copy, obs_val_index, input_qc_index, DART_qc_index, compute_posterior)
...
if(task_count() > model_size) then 
   ...
   call error_handler(E_ERR,'smoother_main', &
...
! call smoother_set_initial_time(init_time_days, init_time_seconds, time1, read_time_from_file)
call smoother_set_initial_time(time1, read_time_from_file)
...
if (perturb_from_single_instance) then
   call error_handler(E_MSG,'smoother_main:', &
...
call smoother_set_window_time(window_time, first_obs_time, last_obs_time)
...
! ===============================================
! The main smoother assimilation starts here ...
! ===============================================
! AdvanceTime : do
   ...
   call smoother_move_ahead(state_ens_handle, ens_size, seq, last_key_used, window_time, &
      key_bounds, num_obs_in_set, curr_ens_time, first_obs_time, last_obs_time)
   ...
   call smoother_sync_keys_time(state_ens_handle, key_bounds, num_obs_in_set, curr_ens_time)

   if(key_bounds(1) < 0) then
      call trace_message('No more obs to assimilate, exiting main loop', 'smoother:', -1)
      ! exit AdvanceTime
      GOTO 100
   endif
   ...
   ! Create an ensemble for the observations plus
   ...
   ! TODO
   call smoother_assim(state_ens_handle, obs_fwd_op_ens_handle, seq, keys, &
   ...
   if(do_ss_inflate(post_inflate)) then
      ...
      if ((inf_sd_initial(2) >= 0.0_r8) .or. inf_sd_initial_from_restart(2)) then
         ...
         call smoother_assim(state_ens_handle, obs_fwd_op_ens_handle, seq, keys, ens_size, num_groups, &
         ...
         ! recalculate standard deviation since this was overwritten in smoother_assim
         call compute_copy_mean_sd(state_ens_handle, 1, ens_size, ENS_MEAN_COPY, ENS_SD_COPY)
...
! end do AdvanceTime
! ===============================================
! The main smoother assimilation ends here ...
! ===============================================

100 call trace_message('End of main assimilation, starting cleanup', 'smoother:', -1)
...
end subroutine smoother_main

! !-----------------------------------------------------------
! !> This mimics the move_ahead() in obs_model_mod.f90
! !> But it is used for smoother. This returns the number of observations
! !> in the assimilation window.
subroutine smoother_move_ahead(ens_handle, ens_size, seq, last_key_used, window_time, &
   key_bounds, num_obs_in_set, curr_ens_time, first_obs_time, last_obs_time)
...
type(time_type),         intent(in)    :: first_obs_time, last_obs_time
type(time_type),         intent(inout) :: curr_ens_time

! type(time_type)    :: next_ens_time
...
call get_ensemble_time(ens_handle, 1, curr_ens_time)

! call print_time(curr_ens_time)
! call print_time(next_time)
! next_ens_time = get_closest_state_time_to(curr_ens_time, next_time)

call get_obs_time_range(seq, first_obs_time, last_obs_time, key_bounds, num_obs_in_set, &
   out_of_range, observation)

call timechat(curr_ens_time,   'smoother_move_ahead', .false.,        'Current model data time       at: ')
call timechat(first_obs_time,  'smoother_move_ahead', .false.,        'Assimilation window starts    at: ')
call timechat(last_obs_time,   'smoother_move_ahead', .false.,        'Assimilation window ends      at: ')
...
end subroutine smoother_move_ahead
...
subroutine smoother_initialize_modules_used()

call trace_message('Before smoother_initialize_module_used call')
...
call initialize_qc()
call trace_message('After smoother_initialize_module_used call')

end subroutine smoother_initialize_modules_used

!-------------------------------------------------------------------------

subroutine smoother_setup_obs_sequence(seq, in_obs_copy, obs_val_index, &
...
if (input_qc_index < 0) then
   ...
   if (input_qc_index < 0) then
      ...
      input_qc_index = get_blank_qc_index(seq)
      if (input_qc_index < 0) then
         call error_handler(E_ERR,'smoother_setup_obs_sequence', &
...
end subroutine smoother_setup_obs_sequence
...
subroutine smoother_set_initial_time(time, read_time_from_file)
! subroutine smoother_set_initial_time(days, seconds, time, read_time_from_file)
...
end subroutine smoother_set_initial_time

!-------------------------------------------------------------------------

subroutine smoother_set_window_time(time, first_obs_time, last_obs_time)

type(time_type), intent(in)  :: first_obs_time, last_obs_time
type(time_type), intent(out) :: time


if(last_obs_days < 0) then
   if (obs_window_days < 0) call error_handler(E_ERR,'smoother_set_window_time','observation window cannot be negative.',source,revision,revdate)
   time = set_time(obs_window_seconds, obs_window_days)
else
   time = last_obs_time - first_obs_time
endif

end subroutine smoother_set_window_time
...
subroutine smoother_sync_keys_time(ens_handle, key_bounds, num_obs_in_set, time1)
...
end subroutine smoother_sync_keys_time
...
if (present(label)) then
   ...
   call error_handler(E_MSG,' smoother trace:',trim(msg))
endif
...
call set_multiple_filename_lists(input_state_files(:), &
                                 ...
                                 ndomains, &
                                 ninput_files,     &
                                 'smoother','input_state_files','input_state_file_list')
call set_multiple_filename_lists(output_state_files(:), &
                                 output_state_file_list(:), &
                                 ndomains, &
                                 noutput_files, &
                                 'smoother','output_state_files','output_state_file_list')
...
! $URL: https://svn-dares-dart.cgd.ucar.edu/DART/releases/Manhattan/assimilation_code/modules/assimilation/filter_mod.f90 $
! $Id: filter_mod.f90 13137 2019-04-30 15:48:00Z nancy@ucar.edu $
! $Revision: 13137 $
