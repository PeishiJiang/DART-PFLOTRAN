! This file is revised from DART assim_tools_mod.f90.
!
! $Id: assim_smoother_tools_mod.f90 13137 2020-03-08 peishi.jiang@pnnl.gov $

!>  A variety of operations required by assimilation.
module assim_smoother_tools_mod
...
character(len=*), parameter :: revision = "$Revision: 13137 $"
character(len=*), parameter :: revdate  = "$Date: 2019-04-30 08:48:00 -0700 (Tue, 30 Apr 2019) $"
...
subroutine smoother_assim(ens_handle, obs_ens_handle, obs_seq, keys,           &
...
type(time_type) :: dummytime
type(time_type), allocatable :: my_obs_time(:)
type(time_type), allocatable :: my_state_time(:)
...
allocate(close_obs_dist(     obs_ens_handle%my_num_vars), &
         ...
         my_obs_time(        obs_ens_handle%my_num_vars))
...
allocate(close_state_dist(     ens_handle%my_num_vars), &
         ...
         my_state_time(        ens_handle%my_num_vars))
! end alloc
...
if (sort_obs_inc) then
   if(deterministic_inflate(inflate) .and. ((filter_kind == 1) .or. (filter_kind == 8))) then
      ...
      call error_handler(E_MSG,'WARNING smoother_assim:', msgstring, source, revision, revdate, &
...
if ((grp_size * num_groups) /= ens_size) then
   ...
   call error_handler(E_ERR,'smoother_assim:', msgstring, source, revision, revdate, &
                         text2=msgstring2,text3=msgstring3)
endif
if (grp_size < 2) then
...
! TODO: revise the get_state_meta_data to allow including time dimension
...
do i = 1, ens_handle%my_num_vars
   ...
   call get_state_meta_data(my_state_indx(i), my_state_loc(i), my_state_kind(i), my_state_time(i))
end do
...
SEQUENTIAL_OBS: do i = 1, obs_ens_handle%num_vars
   ...
   if (nth_obs == 0) then
      ...
      if (print_timestamps == 0) then
         call error_handler(E_MSG,'smoother_assim',msgstring)
      else
   ...
   if (base_obs_type > 0) then
      ...
      call get_state_meta_data(-1 * int(base_obs_type,i8), dummyloc, base_obs_kind, dummytime)  ! identity obs
   endif
...
! Added by Peishi for testing
real(r8) :: pert(ens_size), pert_mean, pert_std, pert_var
...
do i = 1, ens_size
   ...
   !  The following is added by Peishi
   !  temp_obs(i) = obs + i * 0.001 - 1
   !  temp_obs(i) = obs + 1. / ens_size * i - 1. 
   !  temp_obs(i) = obs + i * 0.001
   !  temp_obs(i) = obs
end do
...
!    pert(i) = i
! end do
! pert_mean = sum(pert) / ens_size
! pert_var = sum((pert - pert_mean) ** 2) / (ens_size - 1)
! pert_std = sqrt(pert_var)
! do i = 1, ens_size
!    pert(i) = (pert(i) - pert_mean) / pert_std * sqrt(obs_var)
! end do
! do i = 1, ens_size
!    temp_obs(i) = obs + pert(i)
! end do
! print *, obs, sqrt(obs_var)
! print *, "the variance of prior predicted variable:"
! print *, prior_var
! ! print *, sum((ens - sum(ens) / ens_size)**2) / (ens_size - 1)
! print *, "the variance of perturbed observation:"
! ! print *, pert_var
! print *, sum((temp_obs - sum(temp_obs) / ens_size)**2) / (ens_size - 1)
! print *, "perturbations:"
! print *, pert
! print *, "predictions:"
! print *, ens
! print *, "perturbed observation:"
! print *, temp_obs
...
Get_Obs_Locations: do i = 1, obs_ens_handle%my_num_vars
   ...
   if (my_obs_type(i) > 0) then
      ...
      call get_state_meta_data(-1 * int(my_obs_type(i),i8), dummyloc, my_obs_kind(i), dummytime)
   endif
...
end module assim_smoother_tools_mod

! <next few lines under version control, do not edit>
! $URL: https://svn-dares-dart.cgd.ucar.edu/DART/releases/Manhattan/assimilation_code/modules/assimilation/assim_tools_mod.f90 $
! $Id: assim_tools_mod.f90 13137 2019-04-30 15:48:00Z nancy@ucar.edu $
! $Revision: 13137 $
